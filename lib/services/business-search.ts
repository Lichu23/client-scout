...
export async function searchAndSaveBusinesses(input:{businessType:string;location:string;description?:string;limit:number}) {
 ...
 const leads=await Promise.all(businesses.map(async b=>{
  ...
  const scored=scoreLead({businessType:b.category,requestedType:input.businessType,instagram:Boolean(ig.username),website:Boolean(website),analysis,description:input.description});
  const data={businessName:b.name,businessType:input.businessType,location:input.location,phone:b.phone??null,source:"Google Places",googlePlaceId:existing?.googlePlaceId ?? b.placeId,googleMapsUrl:b.mapsUrl,websiteUrl:analysis?.finalUrl??website?.toString()??null,websiteDomain:domain??null,instagramUrl:ig.url??null,instagramUsername:ig.username??null,hasWebsite:Boolean(website),hasInstagram:Boolean(ig.username),hasHttps:analysis?.hasHttps??null,hasContactForm:analysis?.hasContactForm??null,hasBooking:analysis?.hasBooking??null,hasAppointment:analysis?analysis.hasAppointment:null,hasEcommerce:analysis?.hasEcommerce??null,hasWhatsApp:analysis?.hasWhatsApp??null,websiteAnalyzedAt:analysis?new Date():null,enrichmentStatus:website?(analysis?"complete":"unavailable"):"not_applicable",leadScore:scored.score,scoreReasons:JSON.stringify(scored.reasons),opportunity:scored.opportunity};
  const lead=existing?await db.lead.update({where:{id:existing.id},data}):await db.lead.create({data:{...data,searchId:search.id}});
  await db.searchLead.upsert({where:{searchId_leadId:{searchId:search.id,leadId:lead.id}},create:{searchId:search.id,leadId:lead.id},update:{}});
  return lead;
 })); return {search,leads};
}
